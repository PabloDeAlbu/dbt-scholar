{% set field_relation = source('vocsedici', 'paragraph__field_institucion') %}
{% if execute %}
...
renamed as (
  select
    ...
    {{ safe_cast(field_relation, 'field_institucion_target_id', 'bigint', alias='institution_node_id', col_names=field_col_names, default_mode='ghost') }},
...
casted as (
  select
    ...
    institution_node_id::bigint as institution_node_id,
...
ghost_record as (
  select
    ...
    -1::int as delta,
    -1::bigint as institution_node_id,
