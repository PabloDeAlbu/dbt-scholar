{% set field_relation = source('vocsedici', 'node__field_nombre_institucion') %}
{% if execute %}
...
renamed as (
  select
    ...
    {{ safe_cast(field_relation, 'field_nombre_institucion_value', 'text', alias='institution_name', col_names=field_col_names, default_mode='ghost') }},
...
casted as (
  select
    ...
    institution_name::text as institution_name,
...
ghost_record as (
  select
    ...
    -1::int as delta,
    '!UNKNOWN'::text as institution_name,
