{% set field_relation = source('vocsedici', 'node__field_google_scholar') %}
{% if execute %}
...
renamed as (
  select
    ...
    {{ safe_cast(field_relation, 'field_google_scholar_value', 'text', alias='google_scholar', col_names=field_col_names, default_mode='ghost') }},
...
casted as (
  select
    ...
    google_scholar::text as google_scholar,
...
ghost_record as (
  select
    ...
    -1::int as delta,
    '!UNKNOWN'::text as google_scholar,
