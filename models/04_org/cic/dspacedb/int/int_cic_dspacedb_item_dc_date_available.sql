...
WITH parsed AS (
    SELECT
        ...
        value_raw,
        CASE
            WHEN value ~ '^[0-9]{4}-(0[1-9]|1[0-2])-(0[1-9]|[12][0-9]|3[01])T'
                THEN TO_DATE(LEFT(value, 10), 'YYYY-MM-DD')
        ...
        END AS value,
        CASE
            WHEN value ~ '^[0-9]{4}-(0[1-9]|1[0-2])-(0[1-9]|[12][0-9]|3[01])T' THEN 'day'
...
SELECT item_uuid, metadata_value_id, value_raw, value, value_precision
FROM ranked
WHERE value_rank = 1
