...
WITH base AS (
    ...
    WHERE discoverable IS TRUE
      AND dc_identifier_uri IS NOT NULL
),
...
author_ranked AS (
    SELECT
        ...
        END AS role_order,
        ROW_NUMBER() OVER (
            PARTITION BY author.item_uuid, LOWER(author.author_name)
            ORDER BY
                CASE author.author_role
                    WHEN 'author' THEN 1
                    WHEN 'corporate' THEN 2
    ...
    FROM {{ ref('brg_cic_dspacedb_item_author') }} AS author
    INNER JOIN base
        USING (item_uuid)
),
...
doi_agg AS (
    ...
    FROM {{ ref('int_cic_dspacedb_item_dcterms_identifier_doi') }} AS doi
    INNER JOIN base
        USING (item_uuid)
    GROUP BY doi.item_uuid
),
...
isbn_agg AS (
    ...
    FROM {{ ref('int_cic_dspacedb_item_dcterms_identifier_isbn') }} AS isbn
    INNER JOIN base
        USING (item_uuid)
    GROUP BY isbn.item_uuid
),
...
prepared AS (
    SELECT
        ...
        CASE
            ...
            WHEN LOWER(base.dc_type) IN ('artículo', 'contribucion a revista', 'revisión')
                THEN 'article'
            WHEN LOWER(base.dc_type) = 'libro' THEN 'book'
            WHEN LOWER(base.dc_type) = 'documento de conferencia'
                THEN 'objeto de conferencia'
            ELSE 'unknown'
        END::text AS type,
        ...
        base.owning_collection::text AS owning_collection_id,
