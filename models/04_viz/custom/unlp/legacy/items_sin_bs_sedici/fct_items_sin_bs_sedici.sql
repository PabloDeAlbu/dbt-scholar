...
WITH base as (
	...
	LEFT JOIN {{ref('er_dspace5_metadatavalue')}} origin ON 
		origin.metadatavalue_hk = bridge_mods_origininfo_place.metadatavalue_hk

	LEFT JOIN {{ref('er_dspace5_item_metadatavalue')}} bridge_sedici_identifier_uri ON 
	...
	LEFT JOIN {{ref('er_dspace5_metadatavalue')}} uri ON 
		uri.metadatavalue_hk = bridge_dc_title.metadatavalue_hk

	LEFT JOIN {{ref('er_dspace5_item_metadatavalue')}} bridge_mods_location ON 
		bridge_mods_location.item_hk = i.item_hk AND 
		bridge_mods_location.metadatafield_fullname = 'mods.location'
	LEFT JOIN {{ref('er_dspace5_metadatavalue')}} loc ON 
		loc.metadatavalue_hk = bridge_dc_title.metadatavalue_hk

	INNER JOIN {{ref('er_dspace5_collection')}} c ON
		i.owningcollection_hk = c.collection_hk

	LEFT JOIN {{ref('er_dspace5_item_bitstream')}} i_bs ON
	...
	INNER JOIN {{ ref('er_dspace5_community_metadatavalue') }} brd_com_mv ON 
		brd_com_mv.community_hk = lnk_com_col.community_hk AND brd_com_mv.metadatafield_fullname = 'dc.title'

	INNER JOIN {{ ref('er_dspace5_metadatavalue')}} mv_com ON
