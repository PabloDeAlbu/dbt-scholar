WITH work as (
    SELECT
        ...
        sat_work.title,
        sat_work.type,
        TO_DATE(sat_work.publication_year::text, 'YYYY') as publication_year,
...
openalex_work AS (
    SELECT 
        ...
        work_pid.pmcid,
        work_pid.pmid,
        CASE WHEN work_pid.doi <> '-' THEN TRUE ELSE FALSE END AS has_doi,
        CASE WHEN work_pid.mag <> '-' THEN TRUE ELSE FALSE END AS has_mag,
        CASE WHEN work_pid.pmcid <> '-' THEN TRUE ELSE FALSE END AS has_pmcid,
        CASE WHEN work_pid.pmid <> '-' THEN TRUE ELSE FALSE END AS has_pmid
    FROM work
