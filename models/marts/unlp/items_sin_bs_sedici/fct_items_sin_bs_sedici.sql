...
WITH base as (
	...
	LEFT JOIN {{ref('dim_metadatavalue_dspace5')}} origin ON 
		origin.metadatavalue_hk = bridge_mods_origininfo_place.metadatavalue_hk

	LEFT JOIN {{ref('bridge_item_metadatavalue_dspace5')}} bridge_sedici_identifier_uri ON 
	...
	LEFT JOIN {{ref('dim_metadatavalue_dspace5')}} uri ON 
		uri.metadatavalue_hk = bridge_dc_title.metadatavalue_hk

	LEFT JOIN {{ref('bridge_item_metadatavalue_dspace5')}} bridge_mods_location ON 
		bridge_mods_location.item_hk = i.item_hk AND 
		bridge_mods_location.metadatafield_fullname = 'mods.location'
	LEFT JOIN {{ref('dim_metadatavalue_dspace5')}} loc ON 
	...
	INNER JOIN {{ ref('bridge_community_metadatavalue_dspace5') }} brd_com_mv ON 
		brd_com_mv.community_hk = lnk_com_col.community_hk AND brd_com_mv.metadatafield_fullname = 'dc.title'

	INNER JOIN {{ ref('dim_metadatavalue_dspace5')}} mv_com ON
