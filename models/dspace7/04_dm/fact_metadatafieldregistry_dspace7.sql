...
WITH base AS (
    ...
    FROM {{ref('stg_dspace7_metadatafieldregistry')}} mf
    JOIN {{ref('stg_dspace7_metadataschemaregistry')}} ms ON mf.metadata_schema_id = ms.metadata_schema_id
    LEFT JOIN {{ref('stg_dspace7_metadatavalue')}} mv ON mv.metadata_field_id = mf.metadata_field_id
